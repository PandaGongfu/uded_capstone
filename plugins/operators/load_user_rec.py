...
class LoadUserRecOperator(BaseOperator):
    ui_color = '#358140'
    ...
    def __init__(self,
        ...
        super(LoadUserRecOperator, self).__init__(*args, **kwargs)
        self.table = table
    ...
    def execute(self, context):
        ...
        update_query = f"""
        INSERT INTO graph_recs.{self.table} (user_id, rec_id, type, created_at, updated_at)
        SELECT t.user_id, t.rec_id, 'user', current_timestamp, current_timestamp FROM graph_recs.{self.temp_table} t
        ON CONFLICT (user_id, rec_id, type) 
